...
@Component({
  ...
  styleUrl: './update-subcategories.component.scss',
})
export class UpdateSubcategoriesComponent implements OnInit, OnDestroy {
  subscription: any;
  ...
  loadSubcategory(categoryId: string) {
    this.subscription = this._SubcategoriesService
  ...
  getAllCategories() {
    this.subscriptionCategories = this._CategoriesService
  ...
  updateSubcategory(categoryId: string, formData: FormGroup) {
    this._SubcategoriesService
      .updateSubcategory(categoryId, formData.value)
      .subscribe({
        next: (res) => {
          alert('subcategory updated');
          this._Router.navigate(['/layout/subCategories']);
        },
        error: (err) => {

...
export class ReviewsService {
  // we use hostName towice so I left it
  hostName: string = '';
  routeName: string = '';
  ...
  getUserReviews(
    ...
    return this._HttpClient.get(
      `${this.hostName}${this.routeName}/myReviews/?limit=${limit}&page=${page}&sort=${sort}&search=${search}`,
      { headers: { authorization: `Bearer ${localStorage.getItem('token')}` } }
    );
  }
  ...
  updateUserReview(reviewId: string, formData: any) {
    return this._HttpClient.put(
      `${this.hostName}${this.routeName}/${reviewId}`,
      formData,
      { headers: { authorization: `Bearer ${localStorage.getItem('token')}` } }
    );
  }
